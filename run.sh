...
# Note: on default macOS, g++ actually invokes clang, which works,
#   but I replaced g++ with g++-mp-9 after installing gcc9 via MacPorts.

# To run on Windows using Cygwin64...
#  - install:  https://cygwin.com/install.html
#       making sure to select gcc-g++ and bash
#  - install gnuplot (cygwin's gnuplot didn't easily work) in PowerShell via:
#        winget install --exact --id gnuplot.gnuplot 
#  - add the following bin folders to Path in Windows (your folders may differ)...
#       C:\Program Files\gnuplot\bin
#       C:\cygwin64\bin
#  - in this run.sh, change code to code.exe
#  - in this code and plot.gpl, remove "set term x11 enhanced" stuff (or change to "set term windows enhanced")
#  - in this run.sh, replace   echo \"0 0\"   with   ECHO 0 0
#  - in PowerShell (or Command Prompt), go to directory then type:  bash run.sh


# set to exit immediately if any command returns a non-zero exit status
set -e

g++ -O3 -fwhole-program -march=native code.cpp -o code
...
gnuplot -p -e "set term x11 enhanced; plot 'data.dat' u 9:8 with lines title 'altitude vs. great-circle distance', 100000 title '100 km altitude'"
